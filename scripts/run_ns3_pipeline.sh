...
usage() {
  cat <<'EOF'
Usage:
  EXCEL_FILE=/path/to/HCV_BlastHits.xlsx FASTA_POOL=/path/to/FASTA scripts/run_ns3_pipeline.sh
...
cleanup() {
  if [[ -d "$STAGE_DIR" ]]; then
    mv "$STAGE_DIR" "$STAGE_ARCHIVE/NS3_stage_final" 2>/dev/null || true
  fi
...
if [[ -d "$STAGE_DIR" ]]; then
  mv "$STAGE_DIR" "$STAGE_ARCHIVE/NS3_stage_prev"
fi
mkdir -p "$STAGE_DIR"
