...
STAGE_DIR="$TEMP_ROOT/NS5A_stage"
DISCOVERY_TMP="$REPO_ROOT/temp/find_refid_fastas"
DISCOVERY_JSON="$DISCOVERY_TMP/discovery_ns5a.json"
GT_ALLSTUDIES_JSON="$REPO_ROOT/temp/build_ns5a_gt_allstudies/last_run_summary.json"
SOURCEFEATURES_JSON="$REPO_ROOT/temp/build_ns5a_sourcefeatures_csv/last_run_summary.json"
SOURCEFEATURES_GROUPED_JSON="$REPO_ROOT/temp/build_ns5a_sourcefeatures_grouped_csv/last_run_summary.json"
SUBTYPE_ALLSTUDIES_JSON="$REPO_ROOT/temp/build_ns5a_subtype_allstudies_wseqs/last_run_summary.json"
SUBTYPE_WITH_GT_AA_JSON="$REPO_ROOT/temp/build_ns5a_subtype_with_gt_aa/last_run_summary.json"
COMPLETEPROFILES_JSON="$REPO_ROOT/temp/build_ns5a_completeprofiles_tabspergt/last_run_summary.json"
GT_RAS_JSON="$REPO_ROOT/temp/build_ns5a_gt_ras_profiles/last_run_summary.json"
SUBTYPE_RAS_JSON="$REPO_ROOT/temp/build_ns5a_subtype_ras_profiles_nge10/last_run_summary.json"
mkdir -p "$TEMP_ROOT"
...
rm -rf "$STAGE_DIR"
mkdir -p "$STAGE_DIR"
rm -f "$REPO_ROOT/temp/build_ns5a_sourcefeatures_csv/NS5A_SourceFeatures.csv"
rm -f "$REPO_ROOT/temp/build_ns5a_sourcefeatures_grouped_csv/NS5A_SourceFeatures_Grouped.csv"
